...
2. 保存运行中间结果
   ...
   如果目录不存在，将创建此目录。
   每个张量保存到一个文件，命名方式为 `<tensor_name>(<op_name>_<i/o><n>).<fmt>`，已存在的同名文件将被删除。
   上述格式中每对尖括号内包含中间结果的元信息：

   - `tensor_name`: 张量的名字。对于 onnx 模型，张量的名字在模型中是唯一的。张量名中的 `/` 将替换为 `_`，`.` 将替换为 `-`。
   - `op_name`: 保存这个张量关联的算子的名字。算子名中的 `/` 将替换为 `_`，`.` 将替换为 `-`。
   - `i/o`: 张量作为算子的输入或是输出保存。
   - `n`: 张量作为算子的第几个输入/输出保存。
   - `fmt`: 由参数 `data_file_format` 设置的文件格式。如果设置为 `npy`，将以 npy ver 1 的格式保存，否则将以纯二进制文件的格式保存。
