...
TEST(Graph, Subgraph) {
    ...
    {
        auto outputs = graph.topo().globalOutputs();
        graph.fillEdgeInfo();
        EXPECT_EQ(outputs.size(), 1);
        EXPECT_EQ(outputs[0].info().value, (EdgeInfo{Tensor{DataType::F32, {2, 3, 4, 5}}}));
    }
    {
        auto subgraphs = graph.extract({{graph.topo().nodes()[0]}});
        ...
        auto subTopo = subgraph.info().value.subgraph().graph->topo();
    ...
    graph.reduce();
    ASSERT_EQ(graph.topo().nodes().size(), 1);
    ASSERT_TRUE(graph.topo().nodes()[0].info().value.isOperator());
    {
        auto op = graph.topo().nodes()[0].info();
