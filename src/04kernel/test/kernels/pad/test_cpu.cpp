...
TEST(kernel, PadCpu) {
    // no constant_value
    {
        ...
        PadsShape pads = {1, 1, 1, 1};
        PadType type = PadType::Constant;
        PadInfo info = PadInfo(pads, type, *xTensor, *yTensor, false);
        auto kernel = PadCpu::build(std::move(info));
        ASSERT_TRUE(kernel);
    ...
    // have constant_value
    {
        ...
        PadsShape pads = {1, 1, 1, 1};
        PadType type = PadType::Constant;
        PadInfo info = PadInfo(pads, type, *t1Tensor, *yTensor, true);
        auto kernel = PadCpu::build(std::move(info));
        ASSERT_TRUE(kernel);
        auto res = runtime::Resources();
        auto routine = kernel->lower(res).routine;
